...
Graph.prototype.serialize=function(){
  ...
  for(var i=0; i<this.nodes.length; i++){
    pos[3*i]=this.nodes[i].position.x;
    pos[3*i+1]=this.nodes[i].position.y;
    pos[3*i+2]=this.nodes[i].position.z;

    vel[3*i]=this.nodes[i].velocity.x;
    vel[3*i+1]=this.nodes[i].velocity.y;
    vel[3*i+2]=this.nodes[i].velocity.z;

    ids[i]=this.nodes[i].id;
  }

  for(var i=0; i<this.edges.length; i++){
    source[i]=this.edges[i].source.id;
    target[i]=this.edges[i].target.id;
  }
  return {position: pos, velocity: vel, ids: ids, source: source, target: target}
};
