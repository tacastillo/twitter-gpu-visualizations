...
function edgeDetection (imageData, width, height, opencl) {
    ...
    return convolve(imageData, width, height, mask, 1);
}

function convolveCl(imageData, width, height, mask, pass) {
    // Fast Code Here
    ...
    return kernel.run([numElements], null, args)
        .then(function(){
            return testRepulsion();
        }).then(function() {
            if(pass>1){
                return convolveCl(outputBuffer.read(Uint8Array), width, height, mask, pass-1)
            }else{
...
function testRepulsion(){
    ...
    // console.log(posData);
    return kernel.run([num], null, args)
        .then(function(){
            logger.debug(posBuffer.read(Float32Array));
            return posBuffer.read(Float32Array);
        })
}
